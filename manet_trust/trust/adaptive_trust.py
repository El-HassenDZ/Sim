...
- The manager no longer receives the attacker list or generates behaviour.
  ...
  the simulator; ground truth lives in trust/behaviour.py and the metrics
  that need it live in trust/evaluation.py.
...
# Optional parameters not present in the original config: defaults are
# documented assumptions, override them in config.py.
MOBILITY_REF_SPEED = getattr(config, 'MOBILITY_REF_SPEED', 20.0)   # m/s
...
class SlidingWindowTrustManager:
    ...
    def record_observation(self, observer: int, target: int, timestamp: float,
        ...
        if ev is None:
            ev = self.evidence[key] = PairEvidence()
    ...
    def _evidence_count(self, node_id: int) -> int:
        return sum(ev.total_n for (i, j), ev in self.evidence.items()
                   if j == node_id)
