 ...
 *      `ns3::mtcaodv::BlackholeAodvRoutingProtocol`; the enclosing namespace,
...
namespace mtcaodv
{
NS_OBJECT_ENSURE_REGISTERED(BlackholeAodvRoutingProtocol);

/// UDP Port for AODV control traffic
const uint32_t BlackholeAodvRoutingProtocol::AODV_PORT = 654;
...
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::mtcaodv::DeferredRouteOutputTag")
                                .SetParent<Tag>()
...
//-----------------------------------------------------------------------------
BlackholeAodvRoutingProtocol::BlackholeAodvRoutingProtocol()
    : m_rreqRetries(2),
      m_ttlStart(1),
      m_ttlIncrement(2),
      m_ttlThreshold(7),
      ...
      m_lastBcastTime()
{
    m_nb.SetCallback(MakeCallback(&BlackholeAodvRoutingProtocol::SendRerrWhenBreaksLinkToNextHop, this));
}

TypeId
BlackholeAodvRoutingProtocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::mtcaodv::BlackholeAodvRoutingProtocol")
            .SetParent<Ipv4RoutingProtocol>()
            .SetGroupName("MtcAodv")
            .AddConstructor<BlackholeAodvRoutingProtocol>()
            .AddAttribute("HelloInterval",
                          "HELLO messages emission interval.",
                          TimeValue(Seconds(1)),
                          MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_helloInterval),
                          MakeTimeChecker())
            .AddAttribute("TtlStart",
                          "Initial TTL value for RREQ.",
                          UintegerValue(1),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_ttlStart),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("TtlIncrement",
                          "TTL increment for each attempt using the expanding ring search for RREQ "
                          "dissemination.",
                          UintegerValue(2),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_ttlIncrement),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("TtlThreshold",
                          "Maximum TTL value for expanding ring search, TTL = NetDiameter is used "
                          "beyond this value.",
                          UintegerValue(7),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_ttlThreshold),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("TimeoutBuffer",
                          "Provide a buffer for the timeout.",
                          UintegerValue(2),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_timeoutBuffer),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("RreqRetries",
                          "Maximum number of retransmissions of RREQ to discover a route",
                          UintegerValue(2),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_rreqRetries),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RreqRateLimit",
                          "Maximum number of RREQ per second.",
                          UintegerValue(10),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_rreqRateLimit),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RerrRateLimit",
                          "Maximum number of RERR per second.",
                          UintegerValue(10),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_rerrRateLimit),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("NodeTraversalTime",
                          ...
                          MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_nodeTraversalTime),
                          MakeTimeChecker())
            .AddAttribute(
                "NextHopWait",
                "Period of our waiting for the neighbour's RREP_ACK = 10 ms + NodeTraversalTime",
                TimeValue(MilliSeconds(50)),
                MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_nextHopWait),
                MakeTimeChecker())
            .AddAttribute("ActiveRouteTimeout",
                          "Period of time during which the route is considered to be valid",
                          TimeValue(Seconds(3)),
                          MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_activeRouteTimeout),
                          MakeTimeChecker())
            .AddAttribute("MyRouteTimeout",
                          ...
                          MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_myRouteTimeout),
                          MakeTimeChecker())
            .AddAttribute("BlackListTimeout",
                          "Time for which the node is put into the blacklist = RreqRetries * "
                          "NetTraversalTime",
                          TimeValue(Seconds(5.6)),
                          MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_blackListTimeout),
                          MakeTimeChecker())
            .AddAttribute("DeletePeriod",
                          ...
                          MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_deletePeriod),
                          MakeTimeChecker())
            .AddAttribute("NetDiameter",
                          "Net diameter measures the maximum possible number of hops between two "
                          "nodes in the network",
                          UintegerValue(35),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_netDiameter),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute(
                "NetTraversalTime",
                "Estimate of the average net traversal time = 2 * NodeTraversalTime * NetDiameter",
                TimeValue(Seconds(2.8)),
                MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_netTraversalTime),
                MakeTimeChecker())
            .AddAttribute(
                "PathDiscoveryTime",
                "Estimate of maximum time needed to find route in network = 2 * NetTraversalTime",
                TimeValue(Seconds(5.6)),
                MakeTimeAccessor(&BlackholeAodvRoutingProtocol::m_pathDiscoveryTime),
                MakeTimeChecker())
            .AddAttribute("MaxQueueLen",
                          "Maximum number of packets that we allow a routing protocol to buffer.",
                          UintegerValue(64),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::SetMaxQueueLen,
                                               &BlackholeAodvRoutingProtocol::GetMaxQueueLen),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("MaxQueueTime",
                          "Maximum time packets can be queued (in seconds)",
                          TimeValue(Seconds(30)),
                          MakeTimeAccessor(&BlackholeAodvRoutingProtocol::SetMaxQueueTime,
                                           &BlackholeAodvRoutingProtocol::GetMaxQueueTime),
                          MakeTimeChecker())
            .AddAttribute("AllowedHelloLoss",
                          "Number of hello messages which may be loss for valid link.",
                          UintegerValue(2),
                          MakeUintegerAccessor(&BlackholeAodvRoutingProtocol::m_allowedHelloLoss),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("GratuitousReply",
                          "Indicates whether a gratuitous RREP should be unicast to the node "
                          "originated route discovery.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&BlackholeAodvRoutingProtocol::SetGratuitousReplyFlag,
                                              &BlackholeAodvRoutingProtocol::GetGratuitousReplyFlag),
                          MakeBooleanChecker())
            .AddAttribute("DestinationOnly",
                          "Indicates only the destination may respond to this RREQ.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&BlackholeAodvRoutingProtocol::SetDestinationOnlyFlag,
                                              &BlackholeAodvRoutingProtocol::GetDestinationOnlyFlag),
                          MakeBooleanChecker())
            .AddAttribute("EnableHello",
                          "Indicates whether a hello messages enable.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&BlackholeAodvRoutingProtocol::SetHelloEnable,
                                              &BlackholeAodvRoutingProtocol::GetHelloEnable),
                          MakeBooleanChecker())
            .AddAttribute("EnableBroadcast",
                          "Indicates whether a broadcast data packets forwarding enable.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&BlackholeAodvRoutingProtocol::SetBroadcastEnable,
                                              &BlackholeAodvRoutingProtocol::GetBroadcastEnable),
                          MakeBooleanChecker())
            .AddAttribute("UniformRv",
                          "Access to the underlying UniformRandomVariable",
                          StringValue("ns3::UniformRandomVariable"),
                          MakePointerAccessor(&BlackholeAodvRoutingProtocol::m_uniformRandomVariable),
                          MakePointerChecker<UniformRandomVariable>())
            .AddTraceSource("ForgedReply",
                            "A route reply this node was not entitled to send was emitted.",
                            MakeTraceSourceAccessor(
                                &BlackholeAodvRoutingProtocol::m_forgedReplyTrace),
                            "ns3::mtcaodv::BlackholeAodvRoutingProtocol::ForgedReplyTracedCallback")
            .AddTraceSource("AttackDrop",
                            "A transit packet was discarded by the attack policy.",
                            MakeTraceSourceAccessor(
                                &BlackholeAodvRoutingProtocol::m_attackDropTrace),
                            "ns3::mtcaodv::BlackholeAodvRoutingProtocol::AttackDropTracedCallback");
    return tid;
}

void
BlackholeAodvRoutingProtocol::SetMaxQueueLen(uint32_t len)
{
    m_maxQueueLen = len;
...
void
BlackholeAodvRoutingProtocol::SetMaxQueueTime(Time t)
{
    m_maxQueueTime = t;
...
void
BlackholeAodvRoutingProtocol::DoDispose()
{
...
void
BlackholeAodvRoutingProtocol::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
...
int64_t
BlackholeAodvRoutingProtocol::AssignStreams(int64_t stream)
{
...
void
BlackholeAodvRoutingProtocol::Start()
{
    ...
    m_rreqRateLimitTimer.SetFunction(&BlackholeAodvRoutingProtocol::RreqRateLimitTimerExpire, this);
    m_rreqRateLimitTimer.Schedule(Seconds(1));

    m_rerrRateLimitTimer.SetFunction(&BlackholeAodvRoutingProtocol::RerrRateLimitTimerExpire, this);
    m_rerrRateLimitTimer.Schedule(Seconds(1));
}

Ptr<Ipv4Route>
BlackholeAodvRoutingProtocol::RouteOutput(Ptr<Packet> p,
...
void
BlackholeAodvRoutingProtocol::DeferredRouteOutput(Ptr<const Packet> p,
                                     const Ipv4Header& header,
...
bool
BlackholeAodvRoutingProtocol::RouteInput(Ptr<const Packet> p,
                            const Ipv4Header& header,
...
bool
BlackholeAodvRoutingProtocol::Forwarding(Ptr<const Packet> p,
                            const Ipv4Header& header,
                            UnicastForwardCallback ucb,
                            ErrorCallback ecb)
{
...
void
BlackholeAodvRoutingProtocol::SetIpv4(Ptr<Ipv4> ipv4)
{
    NS_ASSERT(ipv4);
    NS_ASSERT(!m_ipv4);
    ...
    Simulator::ScheduleNow(&BlackholeAodvRoutingProtocol::Start, this);
}

void
BlackholeAodvRoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&BlackholeAodvRoutingProtocol::RecvAodv, this));
    ...
    mac->TraceConnectWithoutContext("DroppedMpdu",
                                    MakeCallback(&BlackholeAodvRoutingProtocol::NotifyTxError, this));
}

void
BlackholeAodvRoutingProtocol::NotifyTxError(WifiMacDropReason reason, Ptr<const WifiMpdu> mpdu)
{
    m_nb.GetTxErrorCallback()(mpdu->GetHeader());
}

void
BlackholeAodvRoutingProtocol::NotifyInterfaceDown(uint32_t i)
{
    NS_LOG_FUNCTION(this << m_ipv4->GetAddress(i, 0).GetLocal());
...
void
BlackholeAodvRoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
...
void
BlackholeAodvRoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    NS_LOG_FUNCTION(this);
...
bool
BlackholeAodvRoutingProtocol::IsMyOwnAddress(Ipv4Address src)
{
...
Ptr<Ipv4Route>
BlackholeAodvRoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
...
void
BlackholeAodvRoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    if (m_rreqCount == m_rreqRateLimit)
    {
        Simulator::Schedule(m_rreqRateLimitTimer.GetDelayLeft() + MicroSeconds(100),
                            &BlackholeAodvRoutingProtocol::SendRequest,
                            this,
                            dst);
        return;
    }
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        Simulator::Schedule(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10)),
                            &BlackholeAodvRoutingProtocol::SendTo,
                            this,
                            socket,
                            packet,
                            destination);
...
void
BlackholeAodvRoutingProtocol::SendTo(Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination)
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, AODV_PORT));
}

void
BlackholeAodvRoutingProtocol::ScheduleRreqRetry(Ipv4Address dst)
{
    NS_LOG_FUNCTION(this << dst);
    ...
    m_addressReqTimer[dst].SetFunction(&BlackholeAodvRoutingProtocol::RouteRequestTimerExpire, this);
...
void
BlackholeAodvRoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
...
bool
BlackholeAodvRoutingProtocol::UpdateRouteLifeTime(Ipv4Address addr, Time lifetime)
{
...
void
BlackholeAodvRoutingProtocol::UpdateRouteToNeighbor(Ipv4Address sender, Ipv4Address receiver)
{
...
void
BlackholeAodvRoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    NS_LOG_FUNCTION(this);
    RreqHeader rreqHeader;
...
void
BlackholeAodvRoutingProtocol::SetAttackBehavior(Ptr<AttackBehavior> behavior)
{
...
Ptr<AttackBehavior>
BlackholeAodvRoutingProtocol::GetAttackBehavior() const
{
    return m_attackBehavior;
}

void
BlackholeAodvRoutingProtocol::SendForgedReply(const RreqHeader& rreqHeader,
...
void
BlackholeAodvRoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
...
void
BlackholeAodvRoutingProtocol::SendReplyByIntermediateNode(RoutingTableEntry& toDst,
                                             RoutingTableEntry& toOrigin,
                                             bool gratRep)
{
    NS_LOG_FUNCTION(this);
    ...
    if (toDst.GetHop() == 1)
    {
        ...
        toNextHop.m_ackTimer.SetFunction(&BlackholeAodvRoutingProtocol::AckTimerExpire, this);
...
void
BlackholeAodvRoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
...
void
BlackholeAodvRoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
...
void
BlackholeAodvRoutingProtocol::RecvReplyAck(Ipv4Address neighbor)
{
    NS_LOG_FUNCTION(this);
    RoutingTableEntry rt;
...
void
BlackholeAodvRoutingProtocol::ProcessHello(const RrepHeader& rrepHeader, Ipv4Address receiver)
{
...
void
BlackholeAodvRoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
...
void
BlackholeAodvRoutingProtocol::RouteRequestTimerExpire(Ipv4Address dst)
{
    NS_LOG_LOGIC(this);
    RoutingTableEntry toDst;
...
void
BlackholeAodvRoutingProtocol::HelloTimerExpire()
{
    NS_LOG_FUNCTION(this);
    Time offset;
...
void
BlackholeAodvRoutingProtocol::RreqRateLimitTimerExpire()
{
    NS_LOG_FUNCTION(this);
    m_rreqCount = 0;
    m_rreqRateLimitTimer.Schedule(Seconds(1));
...
void
BlackholeAodvRoutingProtocol::RerrRateLimitTimerExpire()
{
    NS_LOG_FUNCTION(this);
    m_rerrCount = 0;
    m_rerrRateLimitTimer.Schedule(Seconds(1));
...
void
BlackholeAodvRoutingProtocol::AckTimerExpire(Ipv4Address neighbor, Time blacklistTimeout)
{
    NS_LOG_FUNCTION(this);
...
void
BlackholeAodvRoutingProtocol::SendHello()
{
    NS_LOG_FUNCTION(this);
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        Simulator::Schedule(jitter, &BlackholeAodvRoutingProtocol::SendTo, this, socket, packet, destination);
    }
}

void
BlackholeAodvRoutingProtocol::SendPacketFromQueue(Ipv4Address dst, Ptr<Ipv4Route> route)
{
    NS_LOG_FUNCTION(this);
...
void
BlackholeAodvRoutingProtocol::SendRerrWhenBreaksLinkToNextHop(Ipv4Address nextHop)
{
...
void
BlackholeAodvRoutingProtocol::SendRerrWhenNoRouteToForward(Ipv4Address dst,
                                              uint32_t dstSeqNo,
                                              Ipv4Address origin)
{
    NS_LOG_FUNCTION(this);
...
void
BlackholeAodvRoutingProtocol::SendRerrMessage(Ptr<Packet> packet, std::vector<Ipv4Address> precursors)
{
    NS_LOG_FUNCTION(this);
...
Ptr<Socket>
BlackholeAodvRoutingProtocol::FindSocketWithInterfaceAddress(Ipv4InterfaceAddress addr) const
{
...
Ptr<Socket>
BlackholeAodvRoutingProtocol::FindSubnetBroadcastSocketWithInterfaceAddress(Ipv4InterfaceAddress addr) const
{
    NS_LOG_FUNCTION(this << addr);
...
void
BlackholeAodvRoutingProtocol::DoInitialize()
{
    NS_LOG_FUNCTION(this);
    ...
    if (m_enableHello)
    {
        m_htimer.SetFunction(&BlackholeAodvRoutingProtocol::HelloTimerExpire, this);
