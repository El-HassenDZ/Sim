 ...
 *
 * This file is a derivative of ns-3.48 `src/aodv/model/aodv-routing-protocol.h`.
 ...
 * ---------------------------------------------------------------------------
 */
#ifndef MTC_AODV_BLACKHOLE_AODV_ROUTING_PROTOCOL_H
#define MTC_AODV_BLACKHOLE_AODV_ROUTING_PROTOCOL_H
...
/*
 * The AODV support types are reused from the stock `src/aodv` module rather
 * than forked, so this file must not be the place where their semantics drift.
 * These declarations make the reuse explicit and keep the upstream body of the
 * protocol textually unchanged.
 */
using ns3::aodv::DuplicatePacketDetection;
using ns3::aodv::IdCache;
using ns3::aodv::Neighbors;
using ns3::aodv::QueueEntry;
using ns3::aodv::RequestQueue;
using ns3::aodv::RerrHeader;
using ns3::aodv::RoutingTable;
using ns3::aodv::RoutingTableEntry;
using ns3::aodv::RrepAckHeader;
using ns3::aodv::RrepHeader;
using ns3::aodv::RreqHeader;
using ns3::aodv::TypeHeader;
using ns3::aodv::MessageType;
using ns3::aodv::AODVTYPE_RREQ;
using ns3::aodv::AODVTYPE_RREP;
using ns3::aodv::AODVTYPE_RERR;
using ns3::aodv::AODVTYPE_RREP_ACK;
using ns3::aodv::RouteFlags;
using ns3::aodv::VALID;
using ns3::aodv::INVALID;
using ns3::aodv::IN_SEARCH;
 ...
 */
class BlackholeAodvRoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    /**
    ...
    /// constructor
    BlackholeAodvRoutingProtocol();
    ~BlackholeAodvRoutingProtocol() override;
    ...
    // ------------------------------------------------------------------
    // MTCAODV HOOK: attack policy interface.
    //
    // The routing protocol holds no attacker identity and no attack
    // parameters of its own.  Every malicious decision is delegated to the
    // aggregated policy, so a node is benign exactly when no policy is set.
    // ------------------------------------------------------------------

    /**
     * @brief Attach an attack policy to this routing instance.
     *
     * Passing a null pointer restores unmodified AODV behaviour, which is the
     * regression bridge against the stock `src/aodv` baseline.
     *
     * @param behavior Policy consulted at the two attack hooks, or nullptr.
     */
    void SetAttackBehavior(Ptr<AttackBehavior> behavior);

    /**
     * @brief Return the attached attack policy.
     * @return The aggregated policy, or nullptr for a benign node.
     */
  ...
  private:
    ...
    // ------------------------------------------------------------------
    // MTCAODV HOOK: attack state.  All fields are inert when the policy is
    // null, and none of them is readable by any defensive component.
    // ------------------------------------------------------------------

    /** @brief Attack policy consulted at the two hooks; null for a benign node. */
    Ptr<AttackBehavior> m_attackBehavior;

    /**
     * @brief Emit a route reply that this node is not entitled to send.
     *
     * The advertised destination sequence number, hop count and lifetime come
     * from the policy, never from this class.  The reply is unicast along the
     * reverse route that RecvRequest has just created or refreshed.
     *
     * @param rreqHeader The request being answered.
     * @param toOrigin Reverse route back to the request originator.
     */
    void SendForgedReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin);

    /**
     * @brief Fired when a forged route reply is emitted.
     *
     * Arguments: claimed destination, request originator, advertised
     * destination sequence number.
     */
    TracedCallback<Ipv4Address, Ipv4Address, uint32_t> m_forgedReplyTrace;

    /**
     * @brief Fired when a transit packet is discarded by the attack policy.
     *
     * Arguments: the discarded packet and its IPv4 header.
     */
    TracedCallback<Ptr<const Packet>, const Ipv4Header&> m_attackDropTrace;
...
#endif /* MTC_AODV_BLACKHOLE_AODV_ROUTING_PROTOCOL_H */
